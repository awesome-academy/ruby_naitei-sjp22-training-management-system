...
RSpec.describe Trainee::UserTasksController, type: :controller do
  ...
  before do
    ...
    unless defined?(Settings)
      settings_double = double('Settings')
      user_task_double = double('UserTask')
      status_double = double('Status', not_done: 'not_done')
      
      allow(settings_double).to receive(:user_task).and_return(user_task_double)
      allow(user_task_double).to receive(:status).and_return(status_double)
  ...
  describe "PATCH #update_document" do
    context "when document is attached" do
      let(:file) { fixture_file_upload("sample_document.pdf", "application/pdf") }
      ...
      it "attaches the document and sets success flash" do
        ...
        expect(user_task.reload.documents).to be_attached
      ...
      it "creates new user_task and attaches document" do
        ...
        expect {
          patch :update_document, params: {
            ...
            document: fixture_file_upload("sample_document.pdf", "application/pdf")
          }
  ...
  describe "PATCH #update_status" do
    context "with valid status param" do
      it "toggles from not_done to done when param equals Settings value" do
        ...
        expect(user_task.reload.status).not_to eq(:done)
        expect(flash[:success]).to be_present
        expect(response).to redirect_to(
          trainee_course_subject_path(course_id: course.id, id: subject.id, locale: I18n.locale)
        )
      ...
      it "toggles from done to not_done when status param is different" do
        ...
        expect(user_task.reload.status).to eq("not_done")
        expect(flash[:success]).to be_present
      end
  ...
  describe "PATCH #update_spent_time" do
    context "with present spent_time param" do
      it "updates spent_time and sets success flash" do
        ...
        expect(user_task.reload.spent_time).to eq(15)
        expect(flash[:success]).to be_present
        expect(response).to redirect_to(
          trainee_course_subject_path(course_id: course.id, id: subject.id, locale: I18n.locale)
        )
  ...
  describe "private#make_subject_in_progress" do
    context "when user_subject status is not_started" do
      it "updates to in_progress and sets started_at" do
        ...
        expect(ut_user_subject.status).to eq("in_progress")
        expect(ut_user_subject.started_at).not_to be_nil
      end
    end
  ...
  describe "private methods" do
    ...
    describe "private#load_course_and_subject_id" do
      it "assigns course_id and subject_id if valid" do
        ...
        expect(assigns(:course_id)).to eq(course.id)
        expect(assigns(:subject_id)).to eq(subject.id)
      end
