class UserTask < ApplicationRecord
  ...
  validates :documents,
            content_type: {
              ...
              message: Settings.error_messages.invalid_document_type
            },
            size: {
              less_than: Settings.user_task.max_document_size.megabytes,
              message: t(".document_size_exceeded",
                         size: Settings.user_task.max_document_size.megabytes)
            }

  # Scopes
  scope :completed, -> {where(status: :done)}
  scope :pending, -> {where(status: :not_done)}
  scope :by_user, ->(user) {where(user: user)}
  scope :by_task, ->(task) {where(task: task)}
  scope :by_user_subject, ->(user_subject) {where(user_subject: user_subject)}
